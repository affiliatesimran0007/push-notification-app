...
export const sendNotification = async (subscription, payload) => {
  ...
  try {
    ...
    console.log('Hero Image URL:', payload.image || 'No image')
    ...
    console.log('Payload details:', {
      ...
      imageUrl: payload.image ? payload.image.substring(0, 100) + '...' : 'none'
    })
    
    // Web Push payload size limit is ~4KB
    if (payloadString.length > 4000) {
      console.warn('WARNING: Payload might be too large:', payloadString.length, 'bytes - may fail on some browsers')
    }
...
export const createNotificationPayload = ({
  ...
  // Ensure icon is a full URL (use 512x512 for better quality on high-DPI displays)
  let iconUrl = icon || '/icon-512x512.png'
  ...
  // Check if icon is Base64 encoded (data:image...)
  else if (iconUrl && iconUrl.startsWith('data:')) {
    console.log('Detected Base64 encoded icon, using default icon instead. Base64 icons are not supported in web push.')
    iconUrl = `${appUrl}/icon-192x192.png`
  }
  // Check if icon is SVG
  else if (iconUrl && (iconUrl.endsWith('.svg') || iconUrl.includes('.svg?') || iconUrl.includes('image/svg'))) {
    console.log('Detected SVG icon, using default icon instead. SVG icons are not supported in web push.')
    iconUrl = `${appUrl}/icon-192x192.png`
  }
  ...
  // Check if badge is Base64 encoded
  if (badgeUrl && badgeUrl.startsWith('data:')) {
    console.log('Detected Base64 encoded badge, using default badge instead. Base64 badges are not supported in web push.')
    badgeUrl = `${appUrl}/badge-72x72.png`
  }
  // Convert relative paths to absolute URLs
  ...
  if (imageUrl) {
    ...
    if (imageUrl.startsWith('data:')) {
      console.log('Detected Base64 encoded image, skipping. Base64 images are not supported in web push.')
      imageUrl = undefined
    }
    // Convert relative paths to absolute URLs
    ...
    if (imageUrl) {
      try {
        ...
        if (!url.protocol.startsWith('https')) {
          console.warn('Hero image should use HTTPS:', imageUrl)
        }
        console.log('Hero image URL:', imageUrl)
      } catch (e) {
        console.error('Invalid hero image URL:', imageUrl, e)
        imageUrl = undefined
      }
    }
  }
  ...
  const payload = {
    ...
    data: {
      ...
      platformUrl: appUrl, // for dismiss tracking from any domain
      ...data
    },
    vibrate: [200, 100, 200],
    renotify: true
  ...
  // Add image if provided (hero image for rich notifications)
  if (imageUrl) {
